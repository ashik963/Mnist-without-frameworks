...
def _main_(opt):
    train_data=pd.read_csv(opt.train)
    train_label=pd.read_csv(opt.trainlabel)
    test_data=pd.read_csv(opt.test)
    ...
    pd.DataFrame(np.argmax(cache["A3"],axis=0).reshape(-1,1)).to_csv('test_predictions.csv')
